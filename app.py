...
app.config['TITLE'] = 'School Data Hub'
app.title = 'School Data Hub'
#- APP CONFIG

#TODO: Secret key into an .env file!
...
class API(object):
    __slots__ = ()
    PATCH_PUPIL = '/api/pupil/<internal_id>'
    GET_USERS = '/api/user/all'
    POST_USER = '/api/user'
    PUT_USER = '/api/user/<public_id>'
    DELETE_USER = '/api/user/<public_id>'
    GET_USER_LOGIN = '/api/login'
    POST_PUPIL = '/api/pupil'
...
class AdmonitionSchema(ma.Schema):
    ...
    class Meta:
        fields = ('id', 'admonishedpupil_id', 'admonished_day_id', 'admonition_type',
                  'admonition_reason')
...
class PupilAdmonitionSchema(ma.Schema):
    ...
    class Meta:
        fields = ('id', 'admonishedpupil_id', 'admonished_day', 'admonition_type', 'admonition_reason')
...
class AuthorizationSchema(ma.Schema):
    class Meta:
        fields = ('id', 'description', 'status', 'pupil_id', 'created_by')
...
class PupilSchema(ma.Schema):
    ...
    competencechecks = fields.List(fields.Nested(CompetenceCheckSchema))
    ...
    class Meta:
        fields = ('internal_id', 'credit', 'ogs', 
                  ...
                  'pupilworkbooks', 'pupillists', 'avatar_url', 'special_information', 'authorizations', 'competencechecks')
...
class Image(ma.Schema):
    image = File()

class UserSchema(ma.Schema):
    class Meta:
        fields = ('public_id','name', 'admin', 'password')
...
@app.route(API.PUT_USER, methods=['PUT'])
@app.doc(tags=['User'])
@token_required
...
@app.route(API.GET_USER_LOGIN, methods=['GET'])
#@app.doc(tags=['User'])
def login():
...
@app.route(API.POST_PUPIL, methods=['POST'])
@app.doc(tags=['Pupil'])
...
def add_pupil(current_user):
    ...
    if exists == True:
        ...
        entry = 'POST: ' + API.POST_PUPIL + json.dumps(data, indent=None, sort_keys=True)
        new_log_entry = LogEntry(datetime= log_datetime, user=user, entry= entry)
...
@app.route(API.PATCH_PUPIL, methods=['PATCH'])
@app.doc(tags=['Pupil'])
...
def update_pupil(current_user, internal_id):
    ...
    for key in data:
        ...
        log_datetime = datetime.strptime(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')
        user = current_user.name
        entry = 'PATCH: ' + API.PATCH_PUPIL + json.dumps(data, indent=None, sort_keys=True)
        new_log_entry = LogEntry(datetime= log_datetime, user=user, entry= entry)
        db.session.add(new_log_entry)
...
def delete_pupil(current_user, internal_id):
    ...
    log_datetime = datetime.strptime(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')
    user = current_user.name
    entry = 'DELETE: pupil ' + internal_id
    new_log_entry = LogEntry(datetime= log_datetime, user=user, entry= entry)
    db.session.add(new_log_entry)
...
@app.doc(tags=['Goals'], summary='Get goal categories')
...
@app.doc(tags=['Goals'], summary='Get goal categories in flat JSON')
...
@app.doc(tags=['Goals'], summary='Post a goal for a given gategory')
@token_required
...
@app.doc(tags=['Goals'], summary='Patch a goal for a given gategory from a given pupil')
...
@app.doc(tags=['Goal Checks'], summary='Post a check for a given goal from a given pupil')
@token_required
...
@app.route('/api/goal/check/<check_id>', methods=['PATCH'])
@app.doc(tags=['Goal Checks'], summary='Patch a check for a given goal from a given pupil')
@token_required
...
@app.doc(tags=['Goal Categories'], summary='Post a status for a given catagory from a given pupil')
@token_required
...
@app.doc(tags=['Goal Categories'], summary='Patch a status for a given catagory from a given pupil')
@token_required
...
@app.doc(tags=['Competence Checks'], summary='Get competence list as a JSON tree')
@token_required
...
###############################
#- GET COMPETENCES FLAT JSON *CHECKED*
###############################

@app.route('/api/competence/all/flat', methods=['GET'])
@app.doc(tags=['Competence Checks'], summary='Get competence list as a flat JSON')
@token_required
...
@app.doc(tags=['Competence Checks'], summary='Post a check of a competence from a given pupil')
@token_required
def add_competence_check(current_user, internal_id):
    ...
    new_competence_check = CompetenceCheck(check_id, created_by, created_at,
...
###############################
#- PATCH COMPETENCE CHECK WITH IMAGE *CHECKED*
###############################

@app.route('/api/competence/check/<check_id>', methods=['PATCH'])
@app.doc(tags=['Competence Checks'], summary='Patch with an image a check of a competence from a given pupil')
@token_required
def upload_competence_image(current_user, check_id):
    ...
    if competence_check == None:
        return jsonify({'error': 'This competence check does not exist!'})
    ...
    file.save(file_url)
    if len(str(competence_check.file_url)) > 4:
        os.remove(str(competence_check.file_url))
    competence_check.file_url = file_url
    db.session.commit()
    return jsonify({'message': 'media uploaded succesfully'})

###############################
#- PATCH COMPETENCE CHECK DELETE IMAGE *CHECKED*
###############################

@app.route('/api/competence/nomedia/<check_id>', methods=['PATCH'])
@app.doc(tags=['Competence Checks'], summary='Patch-Delete an image associated to a check of a competence from a given pupil')
@token_required
def delete_competence_image(current_user, check_id):
    competence_check = CompetenceCheck.query.filter_by(check_id = check_id).first()
    if competence_check == None:
        return jsonify({'error': 'This competence check does not exist!'})
    if competence_check.file_url == None:
        return jsonify({'error': 'There is no image for this competence check!'})
    os.remove(str(competence_check.file_url))
    competence_check.file_url = None
    db.session.commit()
    return jsonify({'message': 'The image was deleted!'})
...
@app.doc(tags=['Competence Checks'], summary='Get image associated to a check of a competence from a given pupil')
...
@app.route('/competence/check/<check_id>', methods=['DELETE'])
@app.doc(tags=['Competence Checks'], summary='Delete a check of a competence from a given pupil')
@token_required
def delete_competence_check(current_user, check_id):
    ...
    if not competence_check:
        return jsonify({'message' : 'Competence check does not exist!'}),404
...
@app.doc(tags=['School Lists'], summary='Post a school list including ALL pupils in the database')
@token_required
...
@app.doc(tags=['School Lists'], summary='Post a school list including pupils from an array')
@token_required
...
@app.doc(tags=['School Lists'], summary='Get all school lists')
@token_required
...
@app.doc(tags=['School Lists'], summary='Add pupil(s) to a school list')
@token_required
...
@app.doc(tags=['School Lists'], summary='Delete a school list')
@token_required
...
@app.doc(tags=['Schooldays'], summary='Post a schoolday')
@token_required
...
@app.doc(tags=['Schooldays'], summary='Get all schooldays')
@token_required
...
@app.doc(tags=['Schooldays'], summary='Get a list of schooldays without nested elements')
...
@app.doc(tags=['Schooldays'], summary='Get a schoolday with nested elements')
@token_required
...
###############################
#- DELETE ONE SCHOOLDAY *CHECKED*
###############################

@app.route('/api/schoolday/<date>', methods=['DELETE'])
@app.doc(tags=['Schooldays'], summary='Delete a schoolday with nested elements')
@token_required
...
@app.doc(tags=['Missed Classes'], summary='Post a missed class')
@token_required
...
@app.doc(tags=['Missed Classes'], summary='Get all missed classes')
@token_required
...
@app.doc(tags=['Missed Classes'], summary='Get ONE missed class by id')
@token_required
...
@app.doc(tags=['Missed Classes'], summary='Patch a missed class by pupil_id and date')
@token_required
...
###############################
#- DELETE MISSED CLASS WITH DATE *CHECK*
###############################

@app.route('/api/missedclass/<pupil_id>/<schoolday>', methods=['DELETE'])
@app.doc(tags=['Missed Classes'], summary='Delete a missed class by pupil_id and date')
@token_required
...
@app.doc(tags=['Admonitions'], summary='Post an admonition')
@token_required
...
@app.doc(tags=['Admonitions'], summary='Get all admonitions')
@token_required
...
@app.doc(tags=['Admonitions'], summary='Get an admonition by id')
@token_required
...
###############################
#- PATCH ADMONITION  *CHECKED*
###############################

@app.route('/api/admonition/<id>', methods=['PATCH'])
@app.doc(tags=['Admonitions'], summary='Patch an admonition by id')
@token_required
...
###############################
#- DELETE ADMONITION BY ID *CHECKED*
###############################

@app.route('/api/admonition/<id>', methods=['DELETE'])
@app.doc(tags=['Admonitions'], summary='Delete an admonition by id')
@token_required
...
@app.doc(tags=['Pupil'], summary='PATCH-POST an avatar image of a given pupil')
@token_required
...
##########################
#- GET IMAGE PUPIL AVATAR *CHECKED*
##########################

@app.route('/api/pupil/<internal_id>/avatar', methods=['GET'])
@app.doc(tags=['Pupil'], summary='Get avatar image of a given pupil')
...
#############################
#-POST AUTHORIZATION *CHECKED*
#############################

@app.route('/api/pupil/<internal_id>/authorization', methods=['POST'])
@app.doc(tags=['Authorizations'], summary='Post an authorization for a given pupil')
@token_required
def post_authorization(current_user, internal_id):
    pupil = db.session.query(Pupil).filter(Pupil.internal_id == 
                                                internal_id).first()
    if pupil == None:
        return jsonify({'error': 'This pupil does not exist!'})    
    data = request.get_json()
    description = data['description']
    duplicate = db.session.query(Authorization).filter(Authorization.pupil_id == 
                                                internal_id,
                                                Authorization.description == description).first()
    if duplicate != None:
        return jsonify({'error': 'This authorization already exists!'})
    status = data['status']    
    created_by = current_user.name
    pupil_id = internal_id

    authorization = Authorization(description, status, pupil_id, created_by)
    db.session.add(authorization)
    db.session.commit()
    return authorization_schema.jsonify(authorization)

#############################
#-PATCH AUTHORIZATION *CHECKED*
#############################

@app.route('/api/authorization/<auth_id>', methods=['PATCH'])
@app.doc(tags=['Authorizations'], summary='Patch an authorization for a given pupil')
@token_required
def patch_authorization(current_user, auth_id):
    authorization = db.session.query(Authorization).filter(Authorization.id == 
                                            auth_id).first()
    if authorization == None:
        return jsonify({'error': 'This authorization does not exist!'})
    data = request.get_json()
    for key in data:
        match key:
            case 'description':
                authorization.description = data[key]
            case 'status':
                authorization.status = data[key]
            case 'created_by':
                authorization.created_by = data[key]
    db.session.commit()
    return authorization_schema.jsonify(authorization)
...
@app.route('/api/authorization/<auth_id>', methods=['DELETE'])
@app.doc(tags=['Authorizations'], summary='Delete an authorization for a given pupil')
@token_required
def delete_authorization(current_user, auth_id):
    ...
    db.session.commit()
    return jsonify( {"message": "The authorization was deleted!"})
...
@app.route('/api/import/pupils', methods=['POST'])
@app.doc(tags=['CSV Imports'], summary='Import pupils')
@token_required
def upload_pupils_csv(current_user):
    new_pupils = []
    if request.method == 'POST':
            ...
            #- I'd very much like to jump over the first row to have headings in the csv, now taking them out by hand.
                if db.session.query(exists().where(Pupil.internal_id == row[0])).scalar() == False:
                    ...
                    if row[10] == '':
                        ...
                        print('DATE is ' + row[10])
                        datetime_string = row[10]
                        migration_support_ends= datetime.strptime(datetime_string, '%d.%m.%Y').date(),
                   
                    if row[11] == '':
                        migration_follow_support_ends = None
                    else:
                        print('DATE is ' + row[11])
                        datetime_string = row[11]
                        migration_follow_support_ends = datetime.strptime(datetime_string, '%d.%m.%Y').date(),
                           
                    pupil = Pupil(internal_id=row[0],
                                  	credit=row[1],
                                    
                                    ogs=ogs_value,
                                    individual_development_plan=row[3],
                                    five_years=row[4],
                                    special_needs=row[5],
                                    communication_pupil=row[6],
                                    communication_tutor1=row[7],
                                    communication_tutor2=row[8],
                                    preschool_revision=row[9],
                                    migration_support_ends= migration_support_ends,
                                    migration_follow_support_ends=migration_follow_support_ends,
                                    avatar_url=row[12],
                                    special_information=row[13],)
        ...
        db.session.commit()
        return pupil_schema.jsonify(new_pupils)
    
##########################
#- GET IMPORT CATEGORIES *CHECKED*
##########################

@app.route('/api/import/categories', methods=['POST'])
@app.doc(tags=['CSV Imports'], summary='Import goal categories')
@token_required
...
##########################
#- GET IMPORT COMPETENCES *CHECKED*
##########################

@app.route('/api/import/competences', methods=['POST'])
@app.doc(tags=['CSV Imports'], summary='Import competences')
...
########################
#- POST IMPORT SCHOOLDAYS *CHECKED*
########################
@app.route('/api/import/schooldays', methods=['GET', 'POST'])
@app.doc(tags=['CSV Imports'], summary='Import schooldays')
